...
func (suite *KeeperTestSuite) TestListDemandOrdersByStatus() {
	...
	demandOrders, err := keeper.ListDemandOrders(ctx, commontypes.Status_PENDING)
	suite.Require().NoError(err)
	...
	updatedDemandOrders, err := keeper.ListDemandOrders(ctx, commontypes.Status_FINALIZED)
	suite.Require().NoError(err)
	// Validate that there are exactly demandOrderNum packets in total
	pendingDemandOrders, err := keeper.ListDemandOrders(ctx, commontypes.Status_PENDING)
	suite.Require().NoError(err)
